...
OkHttp listener 通过 `callbackFlow` 转换为状态、delta 和完成事件，
Coordinator 在同一 Flow 中接入 HTTP fallback，ViewModel 直接 collect 更新 Compose state。
...
状态：正在进行。已完成聊天来源降噪、流式消息稳定切换、自动滚动、键盘避让、点击空白收键盘、应用内返回手势和最终消息 Markdown 基础渲染；仍需真机验收和弱网处理。
